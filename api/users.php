...
if ($method === 'GET') {
    ...
    $stmt = $conn->prepare(
        "SELECT user_id, user_username, user_email, user_fname, user_lname, user_contact, created_at
         FROM users
...
if ($method === 'PUT') {
    ...
    $stmt = $conn->prepare(
        "UPDATE users SET user_fname = ?, user_lname = ?, user_contact = ? WHERE user_id = ?"
    );
